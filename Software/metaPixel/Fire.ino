...
//		lastNow = now;
	...
	// this works without mapping from XY coordinates to cells
		...
		// 		uint8_t coolBase = ((DISPLAY_HEIGHT+y)*COOLING)/10;
		// 		uint8_t cooling = random8(coolBase,255-heatCells[offset] );
		// 		Serial <<"Cooling ("<<x<<", "<<y<<") "<<cooling<<endl;
		// 		heatCells[offset] =qsub8(heatCells[offset],5*cooling);
		// 	}
		// }
			// 
	for( int i = 0; i < NUM_LEDS; i++) {
		uint8_t k = random8( (255-heatCells[i])/3.0,(COOLING*2)/(DISPLAY_HEIGHT*100.0) ); 
